    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex flex-wrap gap-3 mb-8">
        {[4, 6, 8, 10, 12, 20, 100].map((s) => (
          <MagneticButton
            key={s}
            onClick={() => quickRoll(s)}
            className="dice-btn px-5 py-3 bg-[#1c1c2a] hover:bg-[#242436] border border-[#3a3a4f] rounded-2xl font-mono text-sm"
          >
            d{s}
          </MagneticButton>
        ))}
        <MagneticButton onClick={quickRollAdv} className="px-4 py-3 text-xs border border-[#3a3a4f] rounded-2xl">
          d20 + Adv
        </MagneticButton>
        <MagneticButton onClick={quickRollDis} className="px-4 py-3 text-xs border border-[#3a3a4f] rounded-2xl">
          d20 + Dis
        </MagneticButton>
        <MagneticButton onClick={quickRollPool} className="px-4 py-3 text-xs border border-orange-800 text-orange-400 rounded-2xl">
          4d6kh3 (stat)
        </MagneticButton>
      </div>

      {/* Custom Roll */}
      <SpotlightCard className="p-6 mb-8 rounded-3xl border border-[#3a3a4f] bg-[#14141f]">
        <div className="flex gap-4 items-end">
          <div className="flex-1">
            <label className="text-xs text-[#9ca3b8]">{t('expression', language)}</label>
            <input
              id="dice-expr"
              defaultValue="2d6+3"
              className="w-full bg-[#1c1c2a] border border-[#3a3a4f] focus:border-orange-500 rounded-2xl px-4 py-3 font-mono text-lg"
            />
          </div>
          <div>
            <label className="text-xs text-[#9ca3b8]">{t('times', language)}</label>
            <input
              id="dice-count"
              type="number"
              defaultValue="1"
              className="w-20 bg-[#1c1c2a] border border-[#3a3a4f] rounded-2xl px-3 py-3 text-center"
            />
          </div>
          <MagneticButton
            onClick={() => {
          ...
          <TiltCard>
            <div className="panel p-6 rounded-3xl border border-orange-500/30 bg-black/40">
              <div className="flex justify-between items-start">
                <div>
                  <div className="text-xs text-[#9ca3b8]">{lastRoll.expr}</div>
                            ...
                            <div>
                              <div className="flex items-baseline gap-3 mt-1">
                                <div className="flex items-baseline gap-1">
                                  <span className={`text-4xl font-mono tabular-nums ${r1 === keptNum ? 'text-orange-400 font-semibold' : 'text-[#9ca3b8] line-through'}`}>{r1}</span>
                                  <span className="text-[#6b7280] text-2xl mx-0.5">/</span>
                                  <span className={`text-4xl font-mono tabular-nums ${r2 === keptNum ? 'text-orange-400 font-semibold' : 'text-[#9ca3b8] line-through'}`}>{r2}</span>
                                </div>
                ...
                <div className="text-right text-sm max-w-[180px] text-[#9ca3b8]">{lastRoll.detail}</div>
              </div>
            </div>
          </TiltCard>
        </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-3">
          ...
          <button onClick={clearDiceHistory} className="text-xs text-[#9ca3b8]">{t('clear', language)}</button>
        </div>
        <div className="panel rounded-2xl p-2 max-h-80 overflow-auto">
          {diceHistory.length === 0 && (
            <div className="p-3 text-[#575c6f]">{t('no_rolls', language)}</div>
          )}
          {diceHistory.map((h, i) => (
            <div 
              key={i} 
              onClick={() => navigator.clipboard.writeText(`${h.expr} → ${h.total}${h.detail ? ' ' + h.detail : ''}`)}
              className={`px-3 py-[5px] flex justify-between text-sm items-center cursor-pointer hover:bg-[#1c1c2a] ${i===0 ? 'text-white' : 'text-[#c5c9d8]'}`}
            >
              <div className="flex items-center gap-2">
                <span className="text-orange-400 mono font-medium">{h.expr}</span>
                <span className="text-[#9ca3b8]">{h.detail || ''}</span>
              </div>
