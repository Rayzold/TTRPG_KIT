  ...
  return (
    <div className="max-w-xl">
      <div className="mb-6">
        ...
        <p className="text-muted text-sm">{t('oracle_subtitle', language)}</p>
      </div>
      ...
      <div className="panel p-5 rounded-2xl mb-4">
        <input
          ...
          className="w-full bg-surface2 border border-border px-4 py-3 rounded-xl mb-3"
      ...
      {result && (
        <SpotlightCard className="p-5">
          <div className="text-sm text-muted mb-1">{t('the_oracle_speaks', language)}</div>
          <div className="text-lg">{result}</div>
          <button onClick={() => navigator.clipboard.writeText(result)} className="mt-3 text-xs px-3 py-1 border border-border rounded">Copy</button>
        </SpotlightCard>
      )}
    </div>
  );
